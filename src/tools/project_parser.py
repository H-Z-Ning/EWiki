...
def save_java_callgraph(project_directory,data_path):
    if not os.path.isabs(project_directory):
        project_directory = os.path.abspath(project_directory)
    # 检查目录是否存在
    if not os.path.isdir(project_directory):
    ...
    if not os.path.isdir(project_directory):
        print(f"Error: Directory '{project_directory}' does not exist or is not a directory")
        exit(1)
    summarize_filename = "java_call_chain.json"
    summarize_file = os.path.join(data_path, summarize_filename)
    # 确保目标目录存在
    os.makedirs(os.path.dirname(summarize_file), exist_ok=True)
    if os.path.exists(summarize_file):
        return
    from .java_callgraph import analyze_java_callgraph
    analyze_java_callgraph(project_directory,summarize_file)




def get_java_callgraph_from_faiss_store(query: str,data_path):
    summarize_file = os.path.join(data_path, "java_call_chain.json")
    if not os.path.isfile(summarize_file):  # 文件不存在
        return {}

    if os.path.getsize(summarize_file) < 0 * 1024:
        summarize_path = summarize_file
        if not os.path.isfile(summarize_path):
            print(f"Warning: Analysis file '{summarize_path}' not found, returning empty dict.")
            return {}
        ...
        answer = query_code(query=query,project_root=data_path,out_path=data_path)
        print("---------------------answer------------------------"+answer)
        return answer
